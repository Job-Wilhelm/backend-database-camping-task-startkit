...
--依照"COURSE_BOOKING"的形式，是在booking資料中增添取消的日期。要用user為王小明、教練為李燕容兩條件去搜尋
...
--參考來源 :　https://stackoverflow.com/questions/73388700/postgresql-find-the-sum-of-two-queries-from-different-tables　
select DISTINCT
	user_id,
	(select sum(purchased_credits) from "CREDIT_PURCHASE" where user_id = (select id from "USER" where name = '王小明'))
	-
	(select count(*) from "COURSE_BOOKING" where user_id = (select id from "USER" where name = '王小明') and status != '課程已取消'
group by user_id) as remaining_credit
from "CREDIT_PURCHASE" where user_id = (select id from "USER" where name = '王小明');
...
--因為做作業時已是12月，以下三題因為購買資料已不是在11月建立的，所以選12月做日期範圍
select 
...
where "CREDIT_PURCHASE".purchase_at >= '2024-12-01' 
and "CREDIT_PURCHASE".purchase_at <= '2024-12-31'
group by 組合包方案名稱;
...
-- 顯示須包含以下欄位： 預約會員人數
select distinct 
count(user_id)as 預約會員人數
from "COURSE_BOOKING"
where "COURSE_BOOKING".created_at >= '2024-12-01' 
and "COURSE_BOOKING".created_at <= '2024-12-31' 
and status != '課程已取消';
